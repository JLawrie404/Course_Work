...
startButton.onclick = ()=>{
    rulesList.classList.add("activeInfo"); //show info box
}
...
restart_quiz.onclick = ()=>{
    ...
    timeRemaining.textContent = "Time Left"; //change the text of timeRemaining to Time Left
...
function showQuetions(index){
    ...
    answerList.innerHTML = option_tag; //adding new div tag inside option_tag
...
function optionSelected(answer){
    ...
    if(userAns == correcAns){ //if user selected option is equal to array's correct answer
        playerScore += 1; //upgrading score value with 1
...
function showResult(){
    ...
    if (playerScore > 3){ // if user scored more than 3
        //creating a new span tag and passing the user score number and total question number
        let scoreTag = '<span>and congrats! , You got <p>'+ playerScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;  //adding new span tag inside score_Text
    }
    else if(playerScore > 1){ // if user scored more than 1
        let scoreTag = '<span>and nice , You got <p>'+ playerScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
    else{ // if user scored less than 1
        let scoreTag = '<span>and sorry , You got only <p>'+ playerScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
}
...
function startTimer(time){
    ...
    function timer(){
        ...
        if(time < 9){ //if timer is less than 9
            ...
            timeCalculator.textContent = "0" + addZero; //add a 0 before time value
        }
